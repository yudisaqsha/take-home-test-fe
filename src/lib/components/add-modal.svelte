<script lang="ts">
  ...
  import type { Category, FormData } from "../types/inventory";
  ...
  function handleSubmit(): void {
    ...
    if (formData.buy_price > formData.sell_price) {
      errorMessage = "Buy price cannot be higher than sell price.";
      return;
    }
    addInventoryData(formData);
  }

  async function addInventoryData(formData: FormData) {
    try {
      await inventoryStore.addItem(formData);
      // console.log("Inventory added:", response.data.item);
...
{#if isOpen}
  <div
    class="modal-overlay fixed inset-0 flex items-center justify-center z-5 bg-opacity-50 backdrop-blur"
    on:click={handleOutsideClick}
  >
    <div class="bg-white rounded-lg w-1/2 p-6" on:click|stopPropagation>
      <div class="flex justify-between items-center mb-4">
        <h2 class="text-xl font-bold">Add New Data</h2>
